...
elseif(isset($_REQUEST['reviewid']))
{
   $review_id = $_REQUEST['reviewid'];
   $quote = new Quote();
   $data = $quote->showsuppprofile($review_id);
   print_r($data);
   return $data;
}
...
elseif(isset($_REQUEST['itfpg']) and $_REQUEST['itfpg']== "customerdetail")
{

    $quote = new Quote();
    $datas = $quote->customerreview($_REQUEST['id']);
    print_r($datas);
    return $datas;
}
...
$review_id = $_REQUEST['reviewid'];
    $quote = new Quote();
    $datas = $quote->customerreview($review_id);
    print_r($datas);
    return $datas;
}

elseif(isset($_REQUEST['itfpg']) and $_REQUEST['itfpg']== "detclosequote")
{

    $quote = new Quote();
    $quote->removeCloseQuote($_REQUEST['quoteid']);

    return true;
}
elseif(isset($_REQUEST['bidonquote']))
{
   $bid_amount = $_REQUEST['bid_amount'];
   $attachment = $_REQUEST['attachment'];
   $bid_desc = $_REQUEST['bid_desc'];
   $quoteid = $_REQUEST['quoteid'];
   $usrid = $_SESSION['FRONTUSER']['id'];
   $quote = new Quote();
   $quote->addbidsonquote($bid_amount,$attachment,$bid_desc,$quoteid,$usrid);
   return true;
}
...
elseif(isset($_REQUEST['itfpg']) and $_REQUEST['itfpg']== "discardBid")
{
    ...
    if($bid['status'] == 0 || $bid['status'] == 2  || $bid['status'] == 1){
