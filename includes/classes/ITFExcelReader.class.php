...
//                                            
	...
	function GetDataInfo($datainfo='',$sheetno="0")
	{
		$tmp_resdata=$datainfo->sheets[$sheetno];
		$itfresdata=array();
		
		for($i=2;$i<=count($tmp_resdata['cells']);$i++)
		{
			$itftmd=array();
			for($j=1;$j<=$tmp_resdata['numCols'];$j++)
			{
				$keyname=isset($tmp_resdata['cells'][1][$j])?$tmp_resdata['cells'][1][$j]:"cell_".$j;
				$itftmd[$keyname]=isset($tmp_resdata['cells'][$i][$j])?$tmp_resdata['cells'][$i][$j]:"";
			}
			
			array_push($itfresdata,$itftmd);
		}
		return $itfresdata;
	}
