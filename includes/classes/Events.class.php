...
class Events
{
	...
	function admin_add($datas)
	{
       ...
       //  $datas['category_id'] = implode(",", $datas['category_id']);
		...
		if(isset($_FILES['main_image']['name']) and !empty($_FILES['main_image']['name']))
		{
			...
			$objimage->save(PUBLICFILE."events/".$fimgname);
  ...
  function count_pages($pdfname) {
	...
	function admin_delete($id)
	{
        ...
        $sql="delete from itf_events where id in(".$id.")";
		...
		function seller_product_delete($id)
	{
        ...
        $sql="delete from itf_events where id in(".$id.") and seller_id='".$_SESSION['FRONTUSER']['id']."'";
		$this->dbcon->Query($sql);
		return $this->dbcon->querycounter;
	}
	...
	function admin_update($datas)
	{
		...
		unset($datas['id']);
		$this->dbcon->Modify('itf_events',$datas,$condition);
	}

    function search($search)
    {
       $sql="select P.*, C.catname, PC.catname , MC.catname  from itf_events P
	...
	{
		$sql="select * from itf_events where seller_id='".$_SESSION['FRONTUSER']['id']."'";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	{
		$sql="select * from itf_events where feature=1 and location='".$id."' order by id desc";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	{
		$sql="select * from itf_events where feature=1 order by id desc";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	{
		$sql="select * from itf_events where status = 1 order by id desc limit 0,3";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	{
		$sql="select * from itf_events where status = 1 and location='".$id."' order by id desc limit 0,3";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	{
		$sql="select * from itf_events  where id='".$id."'";
	...
	{
		$sql="select * from itf_events where id='".$UsId."'";
	...
	{
		$sql="select * from itf_events where id='".$UsId."' and seller_id='".$_SESSION['FRONTUSER']['id']."'";
		return $this->dbcon->Query($sql);
	}
    ...
    {
        $sql="select * from itf_events where event_name='".$name."'";
        return $this->dbcon->Query($sql);
    }
    ...
    {
        echo $sql="select * from itf_events where code='".$code."'";
        return $this->dbcon->Query($sql);
    }
    ...
    {
        $sql="select * from itf_events where id='".$id."' and status=1 ";
        return $this->dbcon->Query($sql);
    }
...
function PublishFeatureBlock($ids)
    ...
    function getProductByCategory($catid,$sort = 'id')
    {
        ...
        $sql="select P.* from itf_events P where P.category_id = '".$catid."' or P.subcat_id='".$catid."' ".$order_by;
	   ...
	   function getProductByAlphabet($catid,$sort = '0-9')
    {
        if($sort == '0-9'){
		 ...
		 $sql="select P.* from itf_events P where P.category_id = '".$catid."' order by id DESC";
        }else{
            $order_by = 'order by P.id asc';
			$sql="select P.* from itf_events P where P.category_id = '".$catid."' AND P.name like '".$this->dbcon->EscapeString($sort)."%'";
        }
        return  $this->dbcon->FetchAllResults($sql);;
    }
      ...
      function getProductByAlphabetSearch($sort = '0-9')
    {
        if($sort == '0-9'){
            $order_by = 'order by P.name desc';
		  $sql="select P.* from itf_events P  order by id DESC";
        }else{
            $order_by = 'order by P.id asc';
		 $sql="select P.* from itf_events P where P.name like '".$this->dbcon->EscapeString($sort)."%'";
        }
...

	{
		...
		$sql="select * from itf_events where event_name like '%".$this->dbcon->EscapeString($txtsearch)."%' or  code like '%".$this->dbcon->EscapeString($txtsearch)."%'";            
		return $this->dbcon->FetchAllResults($sql);
	...
	{
		$sql="select * from itf_events where category_id ='".$catid."' and status=1 order by id desc";            
		return $this->dbcon->FetchAllResults($sql);
	}
			...
			function GetLocationName($id)
	{
	 ...
	 function Add_Buying_Request_Details($datas)
	 {
		//print_r($datas);
			...
			$sellerid = $datas['seller_id'];
			unset($datas['id']);
			$this->dbcon->Insert('itf_events_enquiry',$datas);
			$emailadmin =$this->GetAdminEmail1();
			$selleremail =$this->GetSellerEmail($sellerid);
			$selleremailid = $selleremail['email'];
			$emailadmin1 = $emailadmin['email'];
	    ...
	    $sql="select * from itf_events where status=1" .$wherecondition." order by id"; 
		...
		unset($datas['id']);
		$this->dbcon->Insert('itf_events',$datas);
			}
	 ...
	 function showAllProductFrontbyUser($usid)

		{
			...
			$sql="select * from  itf_events where seller_id='".$usid."' order by id DESC";
		...
		function CheckRequestByCustomer($UsId)
			...
			$sql="select U.* from itf_events U where U.id='".$UsId."' and seller_id='".$_SESSION['FRONTUSER']['id']."' ";
	...
	function showAllEnquiries()
		{
			$sql="select * from  itf_events_enquiry order by id DESC";
			return $this->dbcon->FetchAllResults($sql);
		}
	...
	{
		$sql="select U.* from itf_events_enquiry U where U.id='".$UsId."'";
	 	return $this->dbcon->Query($sql);
	}
	...
	$this->dbcon->Insert('itf_favorites',$datas);
	 ...
	 function showAllFavbyUser($usid)
		...
		$sql="select * from  itf_events where status=1 order by price ASC";
		...
		$sql="select * from  itf_events where status=1 order by price DESC";
		...
		function getProductBySearchBySort($sortby)
    {
      ...
      $sql="select P.* from itf_events P where P.status=1 ".$order_by;
	...
	{
		$sql="select * from itf_events where ad_type='0' order by id desc";
		$datas=$this->dbcon->FetchAllResults($sql);
	 	return $datas;
	}
	...
	function ShowAllProductPaid()
	{
		...
		$sql="select P.*,U.txn_id,U.payment_amount,U.payment_status,U.payer_email from itf_events P
	...
	{
		$sql="select * from itf_events where session_id='".$session_id."'"; 
		return $this->dbcon->Query($sql);
	}
	...
	function showAllProductFrontbyUserIdOrEmailFree($usid,$email)

		{
			...
			$sql="select * from  itf_events where seller_id='".$usid."' or sell_email='".$email."' and ad_type='0' order by id DESC";
	...
	{
		$sql="select * from itf_events where feature='1' and ad_type='1' and package_id='1' order by id desc";
		return $this->dbcon->FetchAllResults($sql);
	...
	{
		 $sql="select * from itf_events where ad_type='1' and package_id='2' order by id desc limit 0,3";
		return $this->dbcon->FetchAllResults($sql);
	...
	function showAllPaidNewProductFront()
		{
			$sql="select * from itf_events where feature='1' and ad_type='1' order by id DESC";
			   return  $this->dbcon->FetchAllResults($sql);;
  ...
  <span style="color:#333;display:inline-block;font-size:14px;font-weight:600;vertical-align:middle">'.$datas['deliver_date'].' | '.$datas['deliver_time'].'</span>  </p>        
        ...
        <table style="border-collapse:collapse;border-spacing:0;padding:0;text-align:left;vertical-align:top;width:100%; margin-left:15px;">
          ...
          $messageuser.= '<tr style="padding:0;text-align:left;vertical-align:top">
              <td style="font-family:sans-serif;font-size:13px;text-align:left;vertical-align:top;word-wrap:break-word">'.$catname1['catname'].' | '. $product['event_name'].' ('.$catname2['catname'].')</td>
