...
class WebsiteBanner 
{
	...
	function admin_addBanner($datas)
	{
		if(isset($_FILES['bannerimage']['name']) and !empty($_FILES['bannerimage']['name']))
		{
			...
			$objimage->save(PUBLICFILE."website_banner/".$fimgname);
			$productimagename=$objimage->createnames;
			$datas['imagename']	=	$productimagename;
		}

		unset($datas['id']);
		$this->dbcon->Insert('itf_website_banner',$datas);
	}
	...
	function admin_updateBanner($datas)
	{
		...
		if(isset($_FILES['bannerimage']['name']) and !empty($_FILES['bannerimage']['name']))
		{
			...
			@unlink(PUBLICFILE."website_banner/".$advertiseinfo["imagename"]);
		}
		
		$condition = array('id'=>$datas['id']);
		
		unset($datas['id']);
		$this->dbcon->Modify('itf_website_banner',$datas,$condition);
	}
	...
	function Banner_deleteAdmin($Id)
	{
		...
		@unlink(PUBLICFILE."website_banner/".$advertisedata["imagename"]);
		
		$sql="delete from itf_website_banner where id in(".$Id.")";	
	...
	{
		$sql="select *  from itf_website_banner";
		return $this->dbcon->FetchAllResults($sql);
	}
	
	
	function showAllBannerfront()
	{
		$sql="select *  from itf_website_banner where status=1";
		return $this->dbcon->FetchAllResults($sql);
	}
	...
	{
		$sql="select U.* from itf_website_banner U where U.id='".$UsId."'";
	...
	{
		$sql="select *  from itf_website_banner where id in(".$Id.")";
		return $this->dbcon->FetchAllResults($sql);
	}
