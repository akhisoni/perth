...
class Newsletter
{
	...
	{	
        ...
        function PublishSubscriber($ids)
	{	
		$infos=$this->CheckSubsciberStatus($ids);
		if($infos['status']=='1')
			$datas=array('status'=>'0');
		else
			$datas=array('status'=>'1');
        ...
        function CheckSubsciberStatus($id)
    {
        $sql="select * from itf_subscriber where id='".$id."'";
        return $this->dbcon->Query($sql);
    }
