...
class Gallery 
{
	...
	function admin_addBanner($datas)
	{
		...
		unset($datas['id']);
		$this->dbcon->Insert('itf_gallery',$datas);
	}
	...
	function admin_updateBanner($datas)
	{
		...
		unset($datas['id']);
		$this->dbcon->Modify('itf_gallery',$datas,$condition);
	}
	...
	function Banner_deleteAdmin($Id)
	{
		...
		$sql="delete from itf_gallery where id in(".$Id.")";	
	...
	{
		$sql="select *  from itf_gallery where status=1";
		return $this->dbcon->FetchAllResults($sql);
	}
	...
	{
		$sql="select U.* from itf_gallery U where U.id='".$UsId."'";
	...
	{
		$sql="select *  from itf_gallery where id in(".$Id.")";
		return $this->dbcon->FetchAllResults($sql);
	}
