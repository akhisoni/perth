...
class ITFMailer {
   ...
   function SetTemplate($templatename) {$this->male_template=$templatename;}
   function SetTo($mailto) { $this->mail_to = $mailto;  }
   function SetFrom($frommail) { $this->from_mail = $frommail; }
   function SetName($names) { $this->from_name=$names; }
   function SetCC($cc) { $this->mail_cc=$cc; }
   function SetBCC($bcc) { $this->mail_bcc=$bcc; }
   function SetSubject($mailsubject) { $this->mail_subject=$mailsubject; }

   function SetBody($bodytxt="",$bodydata=array()) 
   { 
	   $allkeysdata=array_keys($bodydata);
	   foreach($allkeysdata as $k=>$itfdts) $allkeysdata[$k]=strtoupper("{".$itfdts."}");
