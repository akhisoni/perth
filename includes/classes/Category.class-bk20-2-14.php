...
class Category 
{
    ...
    {
        $sql="select *  from itf_category where parent ='".$parent."' and status='1' order by catname limit ".$limit." ";
    ...
    function getBreadcumProduct($catid=0,$level=0){
        $res = $this->CheckCategory($catid);
        if(isset($res['id']) and $level == 0){
            return $this->getBreadcum($res['parent'],$level+1).'/ <a href="'.CreateLink(array('product','itemid'=>'catdetail','id'=>$res['id'])).'">'.$res["catname"].'</a> ';
        }elseif(isset($res['id'])){
            return $this->getBreadcum($res['parent'],$level+1).'/ <a href="'.CreateLink(array('product','parent'=>$res['id'])).'">'.$res["catname"].'</a> ';
        }else{
