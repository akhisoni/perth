    ...
    <div class="container">
		...
		$username = $user->CheckProfileIDUser($product['seller_id']);
            ...
            <div class="card" style="box-shadow: 0 0 10px 0 rgba(0, 0, 0, 0.16);">
                <div class="eventimg">
                     <?php if($imge[0]) { ?>
                    <a href="<?php echo CreateLink(array('events','itemid'=>'detail','name'=>$product['slug'])); ?>"> <img class="card-img-top" src="<?php echo PUBLICPATH."/event_images/".$imge[0]; ?>" alt="Card image cap" /></a>
                     <?php }else {?>
                    <a href="<?php echo CreateLink(array('events','itemid'=>'detail','name'=>$product['slug'])); ?>"> <img class="card-img-top" src="<?php echo PUBLICPATH.'events/no_image.jpg'; ?>" alt="Card image cap" /></a>
                    
                    <?php }?>
                </div>
                <div class="card-body">
                    <h5 class="card-title"><a href="<?php echo CreateLink(array('events','itemid'=>'detail','name'=>$product['slug'])); ?>"><?php echo $product['event_name']; ?></a></h5>
