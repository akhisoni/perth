...
if($_REQUEST['type']=='favorites'){
	
	$allfav=$objProduct->Addfav($id);
	flashMsg("Now this ad has been added in your favorite");
	redirectUrl("index.php?itfpage=product&itemid=detail&id=".$id."");
	}
...
<div class="left-imge-l">
  ...
  <ul class="slides">
  <?php if(file_exists(PUBLICFILE."/products/".$imge[0]) && !empty($imge[0])) { ?>
...
<?php }}else { ?>
<li><img src="<?php echo PUBLICPATH."/products/noimage.jpg"; ?>"   width="252px;" height="176px;"  alt=""></li>  
<?php } ?>
</ul>
</div>
...
<div class="right-imge-l">
  ...
  <p class="det-text"><?php echo $productInfo['logn_desc']; ?></p>
   <p class="det-video"><?php echo $productInfo['video']; ?></p>
  <p class="date-in">
  ...
  <strong>&nbsp;  Location: </strong><?php echo $objloc['name'];?> -<a href="#mydiv" onclick="show('mydiv')">map</a><br>
    <strong>Ad Id</strong>: <?php echo $productInfo['id']; ?>  </p>
     <p class="aud1"><?php if($productInfo['price']=='0'){
                  ...
                  <?php }else {?>
					  ...
					  ?>
                  <?php }?></p>
  <p><a href="<?php echo CreateLink(array('reply','id'=>$productInfo['id'])); ?>"><img src="<?php echo TemplateUrl();?>images/reply-btton.png" width="174" height="40" alt=""/>
  </a>
  
  <?php if($_SESSION['FRONTUSER']['id']){?>
  <a href="<?php echo CreateLink(array('product','itemid'=>'detail&id='.$productInfo['id'],'type'=>'favorites')); ?>">
  <img src="<?php echo TemplateUrl();?>images/add-btn.png" width="187" height="40" alt=""/></a>
   <?php } else {?>
      <a href="#" onClick="alert('Sorry, only registered users can add favorite ads!')"> <img src="<?php echo TemplateUrl();?>images/add-btn.png" width="187" height="40" alt=""/></a>
      <?php } ?>
  </p>
